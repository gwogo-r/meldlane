...
Windows: `sounddevice`. Пишет одновременно микрофон (свой голос) и системный звук (собеседники в звонке) в WAV. Оффлайн, без облака — звук митингов не покидает машину.

Системный звук: штатный Windows-loopback **«Стерео микшер»** (Stereo Mix) — работает без установки чего-либо, если включён в настройках звука; VB-Cable остаётся запасным вариантом, если Stereo Mix недоступен на конкретной машине. У устройств разная нативная частота дискретизации (Stereo Mix — обычно 48000 Hz) и часть (WDM-KS, куда попадает Stereo Mix) не поддерживает блокирующее чтение — `capture/audio.py` пишет через callback-API на нативной частоте устройства и ресемплит до 16000 Hz (`scipy.signal.resample_poly`) перед миксом с микрофоном.

Запись останавливается **по времени** (`capture --seconds N`) или **вручную** (`capture` без `--seconds` пишет открыто, до 4 часов по умолчанию, пока из другого терминала не вызовут `capture-stop`). Остановка — файл-флаг (`out/.capture_stop`), не сигнал ОС: процесс записи проверяет его каждые 0.5 сек и, если он появился, сохраняет уже накопленное аудио вместо того, чтобы терять всю запись при принудительном завершении. Проверено вживую: старт → речь → `capture-stop` из другого вызова → WAV сохранён и распознан.

### 3.2 Transcriber
`faster-whisper` (CTranslate2-бэкенд, локально): WAV → `Transcript` (сегменты с таймкодами, язык). Модель настраивается (`tiny…large`) — компромисс скорость/качество; в разы быстрее оригинального `openai-whisper` на CPU при том же качестве модели, не тянет PyTorch (~450 МБ) как зависимость. Для часовых записей на CPU рекомендован `small` (~40-60 мин обработки); `base` быстрее, но менее точен; `medium`/`large` на CPU почти непрактичны для длинных записей. Транскрипт кэшируется, повторный прогон экстрактора не гоняет модель заново.
...
**Решение (принято и реализовано 2026-07-01): оркестратор над CLI-агентами.** Meldlane не изобретает свой tool-use цикл, а запускает уже существующие CLI-агенты как подпроцессы (`agents/cli_runner.py`): `claude -p ... --output-format json` (Claude Code) и `codex exec ... --json` (Codex). Списывается с подписки пользователя (Claude Pro/Max, ChatGPT), не по цене за токен. Маршрутизация по `Member.provider` из `team.yaml`: `claude-code`/`codex` — реальное исполнение (доступ к файлам/shell в изолированной копии репозитория, `sync_workspace()`), `openrouter` — planning-only через Chat Completions. Модель/эффорт подбираются по `story_points` (`select_effort`). Проверено вживую: Codex написал `tests/test_capacity.py`, тесты прошли.

Отвергнутые альтернативы: свой tool-calling цикл на OpenAI SDK (много работы, ответственность за песочницу) и готовый агентский SDK (привязка к одному провайдеру).
...
- `faster-whisper` (CTranslate2) — транскрибация; `sounddevice` + VB-Cable — захват аудио (Windows)
...
**Фаза 2:**
- ~~AgentExecutor — реальное исполнение задач агентом~~ — сделано раньше плана (см. 3.7).
