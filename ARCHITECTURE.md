...
### 3.1 AudioCapture + Transcriber — вынесены в [meldlane-transcribe](https://github.com/gwogo-r/meldlane-transcribe) (MEL-037)
Захват звука и транскрибация больше не живут в этом репозитории — Meldlane подключает их как обычную pip-зависимость (`meldlane-transcribe @ git+...`) и вызывает `meldlane_transcribe.capture`/`meldlane_transcribe.transcriber` напрямую из `main.py`. Причина выноса и устройство пакета — см. раздел «Сервисы» ниже и README самого пакета; здесь — только то, что важно для интеграции.

Записывает микрофон (свой голос) и системный звук (собеседники) **раздельными дорожками** (`mic.wav`/`system.wav` в папке сессии) — диаризация «я / собеседники» бесплатно, без ML. Системный звук на Windows — WASAPI loopback по умолчанию (работает с любым устройством вывода без Stereo Mix/VB-Cable), с fallback на именованные loopback-устройства. Запись останавливается по времени (`capture --seconds N`) или вручную (`capture-stop` из другого терминала, файл-флаг, накопленное не теряется).

Транскрибация — `faster-whisper` (CTranslate2, локально, без PyTorch). Meldlane транскрибирует каждую дорожку с меткой спикера (`me`/`others`), сливает через `merge_tracks()` по таймлайну и конвертирует в свою модель `Transcript` (см. `main.py:_transcribe`). Для часовых записей на CPU рекомендован `MTRANSCRIBE_MODEL=small` (~40-60 мин обработки); `base` быстрее, но менее точен.
...
Meldlane/
  ...
  # capture + transcriber — внешний пакет meldlane-transcribe (см. 3.1), не здесь
  pipeline/
