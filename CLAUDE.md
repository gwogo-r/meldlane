...
- **Аудио (Windows):** sounddevice + Windows «Стерео микшер» (system audio без VB-Cable)
...
capture/    — audio.py + transcriber.py (faster-whisper, CTranslate2)
...
- `python main.py capture [--seconds N]` — живой звук (mic+system); без `--seconds` пишет, пока не остановишь `capture-stop` из другого терминала
- `python main.py transcribe <wav>` — Whisper → задачи
...
- **Известный долг:**
  - System audio проверен живьём через штатный Windows «Стерео микшер» (не нужен VB-Cable) — `SYSTEM_AUDIO_DEVICE=Стерео микшер` в `.env`.
